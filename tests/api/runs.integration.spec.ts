...
test.beforeAll(async ({ playwright, browser }) => {
  ...
  try {
    ...
    console.warn(
      'Failed to get test user session cookies. Authentication tests might be unreliable or require manual setup. Error:',
      error
    );
    organizerCookieString = 'mock-cookie=mock-organizer-session';
    basicUserCookieString = 'mock-cookie=mock-user-session';
  }
});
...
test.describe('GET /api/runs/[id]', () => {
  ...
  test('should return run details for a valid ID', async () => {
    if (!testRun) {
      test.skip(true, 'Test run not created, skipping test.');
      return;
    }
...
test.describe('PUT /api/runs/[id]', () => {
  ...
  test('should allow an authenticated organizer (owner) to update their run', async () => {
    if (
      ...
      test.skip(
        true,
        'Test run or organizer cookie not available, skipping update test.'
      );
      return;
    }
    const updatedData = {
  ...
  test('should return 400 for invalid update data (e.g., descriptor too short)', async () => {
    if (
      ...
      test.skip(
        true,
        'Test run or organizer cookie not available, skipping validation test.'
      );
      return;
    }
  ...
  test('should return 401 if user is not authenticated', async () => {
    if (!runToUpdate) {
      test.skip(
        true,
        'Test run not created, skipping unauthenticated update test.'
      );
      return;
    }
    const response = await apiContext.put(`/api/runs/${runToUpdate.id}`, {
  ...
  test('should return 403 if a basic user tries to update a run', async () => {
    if (
      ...
      test.skip(
        true,
        'Test run or basic user cookie not available, skipping forbidden update test.'
      );
      return;
    }
    const response = await apiContext.put(`/api/runs/${runToUpdate.id}`, {
  ...
  test('should allow an organizer to update any run', async () => {
    if (
      !runToUpdate ||
      !anotherOrganizerCookieString ||
      anotherOrganizerCookieString.startsWith('mock-cookie')
    ) {
      test.skip(
        true,
        'Test run or another organizer cookie not available, skipping non-owner update test.'
      );
      return;
    }
  ...
  test('should return 404 when trying to update a non-existent run', async () => {
    if (
      ...
      test.skip(
        true,
        'Organizer cookie not available, skipping non-existent update test.'
      );
      return;
    }
  ...
  test('should return 400 for an invalid run ID format during update', async () => {
    if (
      ...
      test.skip(
        true,
        'Organizer cookie not available, skipping invalid ID update test.'
      );
      return;
    }
    const invalidRunId = 'invalid-id-format';
...
test.describe('PUT /api/runs/[id]/rsvp', () => {
  ...
  test('should allow an authenticated user to RSVP YES to a run', async () => {
    if (!runForRsvp || !basicUserForRsvp?.cookie) {
      test.skip(
        true,
        'Test run or basic user cookie not available for RSVP test.'
      );
      return;
    }
  ...
  test('should return 404 if trying to RSVP to a non-existent run', async () => {
    if (!basicUserForRsvp?.cookie) {
      test.skip(true, 'Basic user cookie not available');
      return;
    }
    const nonExistentRunId = 'clxxxxxxxxx0000yyyyyyyyyyyy';
  ...
  test('should allow multiple users to RSVP to the same run', async () => {
    if (
      ...
      test.skip(true, 'Prerequisites not met for multiple RSVP test');
      return;
    }
    // User 1 RSVPs
...
test.describe('POST /api/runs/[id]/attendance', () => {
  ...
  test('should allow an authenticated organizer to mark a user as attended', async () => {
    if (
      ...
      test.skip(
        true,
        'Test run or organizer cookie not available for attendance test.'
      );
      return;
    }
  ...
  test('should be idempotent if marking the same user again', async () => {
    if (
      ...
      test.skip(true, 'Prerequisites not met for idempotency test.');
      return;
    }
    ...
    // Depending on implementation, this could be 200 (OK, already done) or 201 (Created, if it always returns the record)
    // Assuming 200 for existing or 201 if it always tries to create and returns the record.
    // The current implementation plan suggests it handles existing attendance idempotently and likely returns the record.
  ...
  test('should return 403 if a basic user tries to mark attendance', async () => {
    if (
      ...
      test.skip(true, 'Prerequisites not met for basic user attendance test.');
      return;
    }
    const response = await apiContext.post(
      `/api/runs/${runForAttendance.id}/attendance`,
  ...
  test('should return 403 if a non-owner organizer (not admin) tries to mark attendance', async () => {
    ...
    const anotherOrganizerEmail = 'another-org-for-attendance@example.com';
    let tempAnotherOrganizerCookieString: string;
    try {
      tempAnotherOrganizerCookieString =
        await getSessionCookieStringForTestUser(
          test.info().project.use.browserName as any, // This is a bit of a hack to get browser instance in Playwright
          TestUserType.ORGANIZER,
          anotherOrganizerEmail
        );
    } catch (e) {
      console.warn(
        `Failed to get cookie for ${anotherOrganizerEmail} for attendance test, skipping.`
      );
      test.skip(true, `Cookie for ${anotherOrganizerEmail} not available.`);
      return;
    }
    ...
    if (
      !runForAttendance ||
      !tempAnotherOrganizerCookieString ||
      tempAnotherOrganizerCookieString.startsWith('mock-cookie')
    ) {
      test.skip(
        true,
        'Prerequisites not met for non-owner organizer attendance test.'
      );
      return;
    }
    const response = await apiContext.post(
  ...
  test('should return 401 if user is not authenticated to mark attendance', async () => {
    if (!runForAttendance) {
      test.skip(true, 'Test run not created for unauth attendance test.');
      return;
    }
    const response = await apiContext.post(
      `/api/runs/${runForAttendance.id}/attendance`,
  ...
  test('should return 400 for missing userId in request body', async () => {
    if (
      ...
      test.skip(
        true,
        'Prerequisites not met for missing userId attendance test.'
      );
      return;
    }
    const response = await apiContext.post(
  ...
  test('should return 400 for invalid userId format', async () => {
    if (
      ...
      test.skip(
        true,
        'Prerequisites not met for invalid userId format attendance test.'
      );
      return;
    }
    const response = await apiContext.post(
  ...
  test('should return 404 if trying to mark attendance for a non-existent run', async () => {
    if (
      ...
      test.skip(
        true,
        'Organizer cookie not available for non-existent run attendance test.'
      );
      return;
    }
  ...
  test('should return 404 if trying to mark attendance for a non-existent userId', async () => {
    if (
      ...
      test.skip(
        true,
        'Prerequisites not met for non-existent userId attendance test.'
      );
      return;
    }
...
test.describe('POST /api/runs/[id]/photos/confirm-upload', () => {
  ...
  test('should return 403 if a different user tries to confirm the photo', async () => {
    if (
      ...
      !anotherUserCookie ||
      anotherUserCookie.startsWith('mock-cookie')
    ) {
      test.skip(
        true,
